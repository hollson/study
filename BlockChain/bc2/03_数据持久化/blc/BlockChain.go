...
//判断数据库是否存在
func dbExists() bool {
	if _, err := os.Stat(common.NAME_DB); os.IsNotExist(err) {
		return false
	}
	return true
}
...
func CreateGenesisBlockChain(data string) *BlockChain {
	//1. 创建一个区块链，如果数据已存在，则返回最近的区块对象，否则创建创世区块并返回
	if dbExists() {
		db, err := bolt.Open(common.NAME_DB, 0600, nil)
		if err != nil {
			log.Fatal(err)
		}
		//defer db.Close()
		...
		err = db.View(func(tx *bolt.Tx) error {
			b := tx.Bucket([]byte(common.NAME_TABLE))
			if b != nil {
				hash := b.Get([]byte("l")) //读取最后一个hash
	...
	err = db.Update(func(tx *bolt.Tx) error {
		b, err := tx.CreateBucket([]byte(common.NAME_TABLE))
		if err != nil {
			log.Panic(err)
		}
		if b != nil {
			...
			b.Put([]byte("l"), genesisBlock.Hash) //存储最新区块的hash
		}
		return nil
	})
	if err != nil {
...
func (bc *BlockChain) PrintChainV1() {
...
// 递归打印区块链数据
func (bc *BlockChain) PrintChainsV2() {
	//1.获取迭代器对象
